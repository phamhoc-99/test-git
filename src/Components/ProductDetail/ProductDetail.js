...
class ProductDetail extends React.Component {
  ...
  render() {
    console.log(this)
    const idProduct = this.props.match.params.id;//lay id tu duong dan
    const product = this.findProductById(idProduct);
    const user = this.findUserById(product.accountID);
    const account = JSON.parse(sessionStorage.getItem('account'));
    console.log(product)
    const ratingValue = +(product.arrRating ? (product.arrRating.reduce((p, c) => p + c, 0) / product.arrRating.length).toFixed(2) : 0)
